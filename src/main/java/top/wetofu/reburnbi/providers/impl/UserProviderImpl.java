...
import top.wetofu.reburnbi.objects.ReportGroup;
...
public class UserProviderImpl implements UserProvider
{
		...
		try
		{
			...
			{			
				List<ReportUser> list = session.createQuery(
						"from top.wetofu.reburnbi.objects.ReportUser as user "
	...
	{
		String fromClause =
			"from top.wetofu.reburnbi.objects.ReportUser reportUser order by reportUser.name ";
	...
	public void deleteUser(ReportUser user) throws ProviderException
	{
		...
		try
		{
			...
			session
					.createQuery(
							"DELETE top.wetofu.reburnbi.objects.ReportLog reportLog where reportLog.user.id = ? ")
	...
	public List<ReportUser> getUsersForGroup(ReportGroup reportGroup) throws ProviderException
	{
		try
		{
			...
			{			
				List<ReportUser> list = session.createQuery(
						"from top.wetofu.reburnbi.objects.ReportUser as reportUser "
