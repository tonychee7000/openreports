...
public class ReportLogProviderImpl implements ReportLogProvider
{
	...
	{
		String fromClause = "select log.user.name, log.report.name, count(*) from top.wetofu.reburnbi.objects.ReportLog log group by log.user.name, log.report.name order by log.user.name, count(*) desc ";
	...
	{
		String fromClause = "select log.report.name, count(*) from top.wetofu.reburnbi.objects.ReportLog log group by log.report.name order by count(*) desc ";
	...
	{
		String fromClause = "select log.report.name, count(*) from top.wetofu.reburnbi.objects.ReportLog log where log.status = 'failure' group by log.report.name order by count(*) desc ";
	...
	{
		String fromClause = "select log.report.name, count(*) from top.wetofu.reburnbi.objects.ReportLog log where log.status = 'empty' group by log.report.name order by count(*) desc ";
		...
		try
		{
			...
			{			
				List<Object[]> list = session.createQuery(
						"select log.report.name, count(*) from top.wetofu.reburnbi.objects.ReportLog log " 
	...
	{
		String fromClause = "select log.user.name, log.alert.name, count(*) from top.wetofu.reburnbi.objects.ReportLog log group by log.user.name, log.alert.name order by log.user.name, count(*) desc ";
	...
	{
		String fromClause = "select log.alert.name, count(*) from top.wetofu.reburnbi.objects.ReportLog log group by log.alert.name order by count(*) desc ";
	...
	{
		String fromClause = "select log.alert.name, count(*) from top.wetofu.reburnbi.objects.ReportLog log where log.status = 'triggered' group by log.alert.name order by count(*) desc ";
	...
	{
		String fromClause = "select log.alert.name, count(*) from top.wetofu.reburnbi.objects.ReportLog log where log.status = 'not triggered' group by log.alert.name order by count(*) desc ";
