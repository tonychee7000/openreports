...
import top.wetofu.reburnbi.objects.ReportTemplate;
...
public class ReportProviderImpl	implements ReportProvider
{
	...
	{
		String fromClause =
			"from top.wetofu.reburnbi.objects.Report report order by report.name ";
	...
	public void deleteReport(Report report) throws ProviderException
	{
		...
		try
		{
			...
			Iterator<?> iterator =  session
				.createQuery(
					"from  top.wetofu.reburnbi.objects.ReportLog reportLog where reportLog.report.id = ? ")
			...
			iterator =  session
				.createQuery(
						"from top.wetofu.reburnbi.objects.ReportGroup reportGroup").iterate();
